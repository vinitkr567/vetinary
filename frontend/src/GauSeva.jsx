...
// ─── FALLBACK DATA (If Backend is not fully seeded yet) ────────────────────────
const FALLBACK_VETS = [
  { id: 1, name: "Dr. Ramesh Yadav",  initials: "RY", spec: "Bovine Specialist", exp: "15 yrs", loc: "Mathura, UP",   rating: 4.9, reviews: 218, fee: 500,  avail: "Today",    tags: ["Cow","Buffalo","AI Insemination","Emergency"], av_bg: T.greenLight,  av_col: T.greenDark, bio: "Accepting farm visits in Mathura. Specialising in AI insemination, pregnancy diagnosis and general bovine health." },
  { id: 2, name: "Dr. Sunita Devi",   initials: "SD", spec: "Buffalo & Dairy Expert", exp: "10 yrs", loc: "Agra, UP", rating: 4.6, reviews: 134, fee: 450,  avail: "Tomorrow", tags: ["Buffalo","Post-calving","Mastitis"],             av_bg: T.tealLight,   av_col: T.teal, bio: "Specialised in buffalo post-calving care, mastitis treatment, and overall cattle reproductive management." }
];

const FALLBACK_SEMEN = [
...
const CartDrawer = ({ cart, onRemove, onClose }) => {
  ...
  return (
    <div style={{ position: "fixed", right: 0, top: 0, bottom: 0, width: 320, background: T.surface, boxShadow: "-4px 0 24px rgba(0,0,0,0.12)", zIndex: 200, display: "flex", flexDirection: "column" }} className="card-fade-in">
...
// ─── FEED TAB ─────────────────────────────────────────────────────────────────
const FeedTab = ({ vets, listings, feed, onBook, onEnquire }) => {
  const [filter, setFilter] = useState("All");
  const filters = ["All","Vet","Semen","Marketplace","Feed","Medicine"];
  
  const currentVet = vets.length > 0 ? vets[0] : FALLBACK_VETS[0];
  const currentListing = listings.length > 0 ? listings[0] : FALLBACK_LISTINGS[0];
  const feedItems = feed.length > 0 ? feed : FALLBACK_FEED;
  ...
  return (
    <div className="card-fade-in">
      ...
      {/* Vet card */}
      {["All","Vet"].includes(filter) && currentVet && <div style={css.card}>
        <div style={{ ...css.row(10), marginBottom: 10 }}>
          <div style={{ width: 42, height: 42, borderRadius: "50%", background: currentVet.av_bg || T.greenLight, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, color: currentVet.av_col || T.greenDark, fontSize: 14, flexShrink: 0 }}>{currentVet.initials}</div>
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 700, fontSize: 14 }}>{currentVet.name}</div>
            <div style={{ fontSize: 11, color: T.gray }}>{currentVet.spec} · {currentVet.loc}</div>
          </div>
          <Badge label="Available today" color={T.greenDark} bg={T.greenLight} icon="✓" />
        </div>
        <p style={{ fontSize: 13, color: T.text, lineHeight: 1.6, marginBottom: 10 }}>{currentVet.bio || "Govt. registered veterinarian available for farm visits, AI inseminations and checkups."}</p>
        <div style={{ ...css.row(6), flexWrap: "wrap", marginBottom: 10 }}>
          {currentVet.tags ? currentVet.tags.slice(0, 4).map(t => <span key={t} style={css.tag}>{t}</span>) : ["Cow","Buffalo"].map(t => <span key={t} style={css.tag}>{t}</span>)}
        </div>
        <div style={css.divider} />
        <div style={{ ...css.row(12) }}>
          <span style={{ fontSize: 13, color: T.gray }}>⭐ {currentVet.rating} ({currentVet.reviews})</span>
          <div style={{ flex: 1 }} />
          <span style={{ fontWeight: 700, color: T.green, fontSize: 15 }}>₹{currentVet.fee}/visit</span>
          <Btn bg={T.green} onClick={() => onBook(currentVet)}>Book visit →</Btn>
        </div>
      </div>}
      ...
      {["All","Semen"].includes(filter) && <div style={css.card}>
        <div style={{ ...css.row(10), marginBottom: 10 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 11, color: T.gray }}>Verified supplier · Hissar</div>
          </div>
        ...
        <div style={{ ...css.grid2, marginBottom: 10 }}>
          {[["85%","Motility"],["18–20 L","Expected yield"],["95%","Conception rate"],["₹120/straw","240 in stock"]].map(([v,k])=>(
      ...
      {["All","Marketplace"].includes(filter) && currentListing && <div style={css.card}>
        <div style={{ ...css.row(10), marginBottom: 10 }}>
          <div style={{ width: 52, height: 52, borderRadius: 12, background: currentListing.bg || T.greenLight, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28, flexShrink: 0 }}>{currentListing.emoji}</div>
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 700, fontSize: 15 }}>{currentListing.breed} {currentListing.type} — For Sale</div>
            <div style={{ fontSize: 11, color: T.gray }}>{currentListing.loc}</div>
            {currentListing.verified && <Badge label="Verified listing" color={T.greenDark} bg={T.greenLight} icon="✓" />}
          </div>
          <div style={{ fontFamily: "'Sora', sans-serif", fontSize: 20, fontWeight: 700, color: T.green }}>₹{currentListing.price?.toLocaleString()}</div>
        </div>
        <div style={{ ...css.grid2, marginBottom: 10 }}>
          {[[currentListing.yield,"Milk yield"],[currentListing.age,"Age"],[currentListing.lac,"Lactation"],[currentListing.status,"Status"]].map(([v,k])=>(
        ...
        <div style={css.row(8)}>
          <Btn bg={T.green} onClick={() => onEnquire(currentListing)}>Enquire →</Btn>
      ...
      {["All","Feed"].includes(filter) && <div style={css.card}>
        <div style={{ ...css.row(10), marginBottom: 10 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 11, color: T.gray }}>Verified seller · Lucknow</div>
          </div>
...
// ─── MARKETPLACE TAB ─────────────────────────────────────────────────────────
const MarketTab = ({ listings, onRefresh, onEnquire }) => {
  const [filter, setFilter] = useState("All");
  ...
  const filtered = displayListings.filter(l => {
    ...
    if (filter === "Cow") return l.type?.toLowerCase() === "cow";
    if (filter === "Buffalo") return l.type?.toLowerCase() === "buffalo";
    if (filter === "Pregnant") return l.status?.toLowerCase() === "pregnant";
  ...
  return (
    <div className="card-fade-in">
      ...
      {showSell ? <SellForm onRefresh={onRefresh} onBack={() => setShowSell(false)} /> : (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 12 }}>
          {filtered.map(l => (
            <div key={l.id} style={{ ...css.card, padding: 0, overflow: "hidden", marginBottom: 0 }}>
              <div style={{ height: 110, background: l.bg || "#EAF3DE", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 56, position: "relative" }}>
                {l.emoji}
...
// ─── SELL ANIMAL FORM ─────────────────────────────────────────────────────────
const SellForm = ({ onRefresh, onBack }) => {
  const [submitted, setSubmitted] = useState(false);
  const [type, setType] = useState("Cow");
  const [breed, setBreed] = useState("HF / Holstein");
  const [age, setAge] = useState("");
  const [lac, setLac] = useState("1st");
  const [dailyYield, setDailyYield] = useState("");
  const [price, setPrice] = useState("");
  const [status, setStatus] = useState("Healthy — vaccinated");
  ...
  const handleSubmit = async () => {
    if (!type || !breed || !age || !dailyYield || !price || !loc || !farmerName || !farmerMobile) {
    ...
    const payload = {
      ...
      yield: `${dailyYield} L/day`,
      age: `${age} yrs`,
      lac,
      status,
      loc,
      contact_name: farmerName,
      contact_mobile: farmerMobile,
      description: desc
    };
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert("Failed to submit animal listing.");
      }
    } catch(e) {
      console.error(e);
      alert("Error posting animal listing.");
    }
  };

  if (submitted) return (
    <div style={{ textAlign: "center", padding: "40px 20px" }} className="card-fade-in">
      ...
      <div style={{ display: "inline-block", padding: "8px 20px", background: T.blueLight, borderRadius: 9, fontFamily: "'Sora', sans-serif", fontSize: 17, fontWeight: 700, color: T.blue, marginBottom: 20 }}>LST-GAU-{Math.floor(Math.random()*9000+1000)}</div>
      <div><Btn bg={T.green} onClick={onBack}>← Back to listings</Btn></div>
    </div>
  );

  return (
    <div style={css.card} className="card-fade-in">
      <div style={css.cardTitle}>📋 Post a New Listing</div>
      <div style={css.grid2}>
        <FG label="Animal type" required>
          <Select options={["Cow","Buffalo","Calf","Bull"]} value={type} onChange={e => setType(e.target.value)} />
        </FG>
        <FG label="Breed" required>
          <Select options={["HF / Holstein","Jersey","Sahiwal","Gir","Murrah Buffalo","Surti Buffalo","Other"]} value={breed} onChange={e => setBreed(e.target.value)} />
        </FG>
      </div>
      <div style={css.grid2}>
        <FG label="Age (years)" required>
          <Input type="number" placeholder="e.g. 4" value={age} onChange={e => setAge(e.target.value)} />
        </FG>
        <FG label="Lactation no.">
          <Select options={["1st","2nd","3rd","4th","5th+","Pregnant / dry"]} value={lac} onChange={e => setLac(e.target.value)} />
        </FG>
      </div>
      <div style={css.grid2}>
        <FG label="Daily milk yield (L)" required>
          <Input type="number" placeholder="e.g. 14" value={dailyYield} onChange={e => setDailyYield(e.target.value)} />
        </FG>
        <FG label="Asking price (₹)" required>
          <Input type="number" placeholder="e.g. 65000" value={price} onChange={e => setPrice(e.target.value)} />
        </FG>
      </div>
      <FG label="Health status">
        <Select options={["Healthy — vaccinated","Healthy — not vaccinated","Pregnant","Post-calving","Under treatment"]} value={status} onChange={e => setStatus(e.target.value)} />
      </FG>
      <FG label="Location" required>
        <Input placeholder="Village, Block, District, State" value={loc} onChange={e => setLoc(e.target.value)} />
      </FG>
      <FG label="Description">
        <textarea style={{ ...css.input, minHeight: 80, resize: "vertical" }} placeholder="Feeding habits, temperament, vaccination history..." value={desc} onChange={e => setDesc(e.target.value)} />
      </FG>
      <div style={css.grid2}>
        <FG label="Your name" required>
          <Input placeholder="Full name" value={farmerName} onChange={e => setFarmerName(e.target.value)} />
        </FG>
        <FG label="Mobile number" required>
          <Input type="tel" placeholder="+91 XXXXX XXXXX" value={farmerMobile} onChange={e => setFarmerMobile(e.target.value)} />
        </FG>
      </div>
      <div style={{ ...css.row(10) }}>
        <Btn bg={T.surface} color={T.gray} outline full onClick={onBack}>Cancel</Btn>
        <Btn bg={T.green} full onClick={handleSubmit}>Submit listing →</Btn>
      </div>
    </div>
  );
};
...
// ─── SEMEN TAB ───────────────────────────────────────────────────────────────
const SemenTab = ({ semen, onAddCart }) => {
  const [filter, setFilter] = useState("All");
  const breeds = ["All","HF / Holstein","Murrah Buffalo","Sahiwal","Jersey","Gir"];
  
  const displaySemen = semen.length > 0 ? semen : FALLBACK_SEMEN;
  ...
  return (
    <div className="card-fade-in">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 12 }}>
        {displaySemen.filter(s => filter === "All" || s.breed?.includes(filter.split(" ")[0])).map(s => (
          <div key={s.id} style={{ ...css.card, marginBottom: 0 }}>
            <div style={{ display: "inline-block", padding: "3px 10px", borderRadius: 20, background: s.bg || T.greenLight, fontWeight: 700, fontSize: 11, color: T.greenDark, marginBottom: 10 }}>{s.breed}</div>
            <div style={{ ...css.row(10), marginBottom: 10 }}>
...
// ─── FEED/FOOD TAB ───────────────────────────────────────────────────────────
const FodderTab = ({ feed, onAddCart }) => {
  const displayFeed = feed.length > 0 ? feed : FALLBACK_FEED;
  return (
    <div className="card-fade-in">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: 12 }}>
        {displayFeed.map(f => (
          <div key={f.id} style={{ ...css.card, textAlign: "center", marginBottom: 0 }}>
            ...
            <div style={{ fontSize: 12, color: T.gray, marginBottom: 12, lineHeight: 1.5 }}>{f.description || f.desc}</div>
...
// ─── MEDICINE TAB ─────────────────────────────────────────────────────────────
const MedicineTab = ({ medicine, onAddCart }) => {
  const displayMed = medicine.length > 0 ? medicine : FALLBACK_MEDICINE;
  return (
    <div className="card-fade-in">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: 12 }}>
        {displayMed.map(m => (
          <div key={m.id} style={{ ...css.card, textAlign: "center", marginBottom: 0 }}>
            ...
            <div style={{ fontSize: 12, color: T.gray, marginBottom: 12, lineHeight: 1.5 }}>{m.description || m.desc}</div>
...
// ─── VET TAB ──────────────────────────────────────────────────────────────────
const VetTab = ({ vets, onBook }) => {
  const [filter, setFilter] = useState("All");
  const filters = ["All","Cow Specialist","Buffalo Specialist","AI Expert","Emergency","Govt. Approved"];
  
  const displayVets = vets.length > 0 ? vets : FALLBACK_VETS;

  const filtered = displayVets.filter(v => {
    if (filter === "All") return true;
    if (filter === "Cow Specialist") return v.spec?.toLowerCase().includes("cow") || v.spec?.toLowerCase().includes("bovine");
    if (filter === "Buffalo Specialist") return v.spec?.toLowerCase().includes("buffalo") || v.spec?.toLowerCase().includes("dairy");
    if (filter === "AI Expert") return v.tags?.some(t => t.toLowerCase().includes("ai") || t.toLowerCase().includes("insemination"));
    if (filter === "Emergency") return v.tags?.some(t => t.toLowerCase().includes("emergency"));
    if (filter === "Govt. Approved") return v.tags?.some(t => t.toLowerCase().includes("govt") || t.toLowerCase().includes("approved"));
    return true;
  });
      ...
      {filtered.map(v => (
        <div key={v.id} style={css.card}>
          <div style={{ ...css.row(12), marginBottom: 10 }}>
            <div style={{ width: 52, height: 52, borderRadius: "50%", background: v.av_bg || T.greenLight, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 16, color: v.av_col || T.greenDark, flexShrink: 0 }}>{v.initials}</div>
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 12, color: T.amber, marginTop: 2 }}>{"★".repeat(Math.floor(v.rating || 5))} <span style={{ color: T.gray }}>({v.reviews || 0} reviews)</span></div>
            </div>
            <Badge label={v.avail} color={v.avail === "Today" ? T.greenDark : T.blue} bg={v.avail === "Today" ? T.greenLight : T.blueLight} />
          </div>
          <div style={{ ...css.row(6), flexWrap: "wrap", marginBottom: 10 }}>
            {v.tags ? v.tags.map(t => <span key={t} style={css.tag}>{t}</span>) : null}
          </div>
          <div style={css.divider} />
          <div style={css.row(10)}>
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert("Failed to confirm booking. Server returned error.");
      }
    } catch (e) {
      console.error(e);
      alert("Error linking with the booking API.");
    }
  };

  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", zIndex: 300, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      <div style={{ background: T.surface, borderRadius: 14, width: "100%", maxWidth: 480, maxHeight: "90vh", overflowY: "auto" }} className="card-fade-in">
        <div style={{ ...css.row(10), padding: "16px 20px", borderBottom: `1px solid ${T.border}` }}>
          <div>
        ...
        {step === 1 && <div style={{ padding: 20 }}>
          ...
          <div style={css.grid2}>
            <FG label="No. of animals"><Input type="number" placeholder="e.g. 4" value={animalCount} onChange={e => setAnimalCount(e.target.value)} /></FG>
            <FG label="Farmer name" required><Input placeholder="Full name" value={farmerName} onChange={e => setFarmerName(e.target.value)} /></FG>
          </div>
          <FG label="Mobile number" required><Input type="tel" placeholder="+91 XXXXX XXXXX" value={farmerMobile} onChange={e => setFarmerMobile(e.target.value)} /></FG>
          <FG label="Reason for visit">
            <Select options={["General health checkup","Artificial Insemination (AI)","Pregnancy diagnosis","Vaccination","Illness / Injury","Deworming","Post-calving care","Mastitis treatment","Emergency"]} value={reason} onChange={e => setReason(e.target.value)} />
          </FG>
          <FG label="Farm / village address"><Input placeholder="Village, Block, District" value={address} onChange={e => setAddress(e.target.value)} /></FG>
          <FG label="Preferred date" required><Input type="date" value={bookingDate} onChange={e => setBookingDate(e.target.value)} /></FG>
          ...
          <Btn bg={T.green} full onClick={handleConfirm}>Confirm Appointment →</Btn>
        </div>}
    ...
    try {
      const response = await fetch('/api/enquiries', {
        ...
        body: JSON.stringify({
          ...
          name,
          mobile,
          message: msg || `Interested in buying your ${listing.breed} ${listing.type}.`
        })
      });
      if (response.ok) {
        setSent(true);
      } else {
        alert("Enquiry failed. Server returned error.");
      }
    } catch(e) {
      console.error(e);
      alert("Error sending enquiry to API.");
    }
  };

  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", zIndex: 300, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      <div style={{ background: T.surface, borderRadius: 14, width: "100%", maxWidth: 420, padding: 24 }} className="card-fade-in">
        <div style={{ ...css.row(0), justifyContent: "space-between", marginBottom: 16 }}>
          <div style={{ fontFamily: "'Sora', sans-serif", fontWeight: 700, fontSize: 16 }}>Send Enquiry</div>
          <button onClick={onClose} style={{ background: "none", border: "none", fontSize: 22, cursor: "pointer", color: T.gray }}>×</button>
        </div>

        {!sent ? <>
          <div style={{ ...css.row(10), background: listing.bg || T.greenLight, borderRadius: 10, padding: "10px 14px", marginBottom: 16 }}>
          ...
          <FG label="Your name" required><Input placeholder="Full name" value={name} onChange={e => setName(e.target.value)} /></FG>
          <FG label="Mobile number" required><Input type="tel" placeholder="+91 XXXXX XXXXX" value={mobile} onChange={e => setMobile(e.target.value)} /></FG>
          <FG label="Your message">
            <textarea style={{ ...css.input, minHeight: 80, resize: "vertical" }} placeholder={`I am interested in this ${listing.type}. Please share more details...`} value={msg} onChange={e => setMsg(e.target.value)} />
          </FG>
          <Btn bg={T.green} full onClick={handleEnquire}>Send Enquiry →</Btn>
...
const OnboardTab = ({ onRefresh }) => {
  ...
  const [address, setAddress] = useState("");
  const [radius, setRadius] = useState("Up to 20 km");

  const [degree, setDegree] = useState("BVSc & AH");
  const [institution, setInstitution] = useState("");
  const [gradYear, setGradYear] = useState("");
  const [experience, setExperience] = useState("");
  const [regNo, setRegNo] = useState("");
  const [council, setCouncil] = useState("UP State Veterinary Council");
  ...
  const handleSubmit = async () => {
    const initials = name.split(" ").map(w => w[0]).join("").toUpperCase().slice(0, 2) || "RY";
    const payload = {
      name,
      initials,
      spec: `${degree} · Bovine Expert`,
      exp: `${experience} yrs`,
      loc: `${district}, ${state}`,
      fee: parseInt(fee),
      avail: `Mon-Sat`,
    ...
    try {
      const response = await fetch('/api/vets/onboard', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload)
      });
      if (response.ok) {
        ...
        alert("Veterinarian onboarding failed. Server error.");
      }
    } catch(e) {
      console.error(e);
      alert("Error submitting vet onboarding details.");
    }
  };
  ...
  if (done) return (
    <div style={{ ...css.card, textAlign: "center", padding: "40px 20px" }} className="card-fade-in">
  ...
  return (
    <div style={{ maxWidth: 580, margin: "0 auto" }} className="card-fade-in">
      <Stepper />
      ...
      {step === 1 && <>
        <div style={css.card}>
          ...
          <div style={css.grid2}>
            <FG label="Full name" required><Input placeholder="Dr. Ramesh Yadav" value={name} onChange={e => setName(e.target.value)} /></FG>
            <FG label="Mobile number" required><Input type="tel" placeholder="+91 XXXXX XXXXX" value={mobile} onChange={e => setMobile(e.target.value)} /></FG>
          </div>
          <div style={css.grid2}>
            <FG label="Email address"><Input type="email" placeholder="doctor@email.com" value={email} onChange={e => setEmail(e.target.value)} /></FG>
            <FG label="Gender" required>
              <Select options={["Male","Female","Prefer not to say"]} value={gender} onChange={e => setGender(e.target.value)} />
            </FG>
          </div>
        ...
        <div style={css.card}>
          ...
          <div style={css.grid2}>
            <FG label="State" required>
              <Select options={["Uttar Pradesh","Punjab","Haryana","Rajasthan","Madhya Pradesh","Maharashtra","Gujarat","Bihar","West Bengal","Other"]} value={state} onChange={e => setState(e.target.value)} />
            </FG>
            <FG label="District" required><Input placeholder="e.g. Mathura" value={district} onChange={e => setDistrict(e.target.value)} /></FG>
          </div>
          <FG label="Clinic / farm visit address"><Input placeholder="Village, block, pin code" value={address} onChange={e => setAddress(e.target.value)} /></FG>
          <FG label="Service radius" hint="Set how far you're willing to travel for farm visits">
            <Select options={["Up to 10 km","Up to 20 km","Up to 30 km","Up to 50 km","Up to 100 km"]} value={radius} onChange={e => setRadius(e.target.value)} />
          </FG>
        </div>
        <Btn bg={T.green} full onClick={() => {
          if (!name || !mobile || !district) {
            alert("Name, mobile number and district are required!");
            return;
          }
          setStep(2);
        }}>Next — Qualifications →</Btn>
      </>}
      ...
      {step === 2 && <>
        <div style={css.card}>
          ...
          <div style={css.grid2}>
            <FG label="Highest degree" required>
              <Select options={["BVSc & AH","MVSc","PhD (Veterinary)","Diploma in AH"]} value={degree} onChange={e => setDegree(e.target.value)} />
            </FG>
            <FG label="Graduating institution" required><Input placeholder="e.g. IVRI Bareilly" value={institution} onChange={e => setInstitution(e.target.value)} /></FG>
          </div>
          <div style={css.grid2}>
            <FG label="Year of graduation" required><Input type="number" placeholder="2010" value={gradYear} onChange={e => setGradYear(e.target.value)} /></FG>
            <FG label="Years of experience" required><Input type="number" placeholder="15" value={experience} onChange={e => setExperience(e.target.value)} /></FG>
          </div>
          <div style={css.grid2}>
            <FG label="Reg. No." required><Input placeholder="UP/VET/2010/4521" value={regNo} onChange={e => setRegNo(e.target.value)} /></FG>
            <FG label="State Vet Council" required>
              <Select options={["UP State Veterinary Council","Punjab Veterinary Council","Haryana Veterinary Council","Maharashtra Veterinary Council","Gujarat Veterinary Council","VCI (Central)","Other"]} value={council} onChange={e => setCouncil(e.target.value)} />
            </FG>
          </div>
          <FG label="Additional certifications"><Input placeholder="e.g. AI technician, BAIF trained..." value={certs} onChange={e => setCerts(e.target.value)} /></FG>
        </div>
        <div style={css.card}>
        ...
        <div style={css.row(10)}>
          ...
          <Btn bg={T.green} full onClick={() => {
            if (!institution || !gradYear || !experience || !regNo) {
              alert("Graduating institution, graduation year, experience, and registration number are required!");
              return;
            }
            setStep(3);
          }}>Next — Speciality →</Btn>
      ...
      {step === 3 && <>
        ...
        <div style={css.card}>
          ...
          <FG label="Short bio (shown on your profile)">
            <textarea style={{ ...css.input, minHeight: 90, resize: "vertical" }} placeholder="Describe your experience and approach to animal care..." value={bio} onChange={e => setBio(e.target.value)} />
          </FG>
        </div>
        <div style={css.row(10)}>
      ...
      {step === 4 && <>
        ...
        <div style={css.card}>
          ...
          <div style={{ ...css.row(10), padding: "8px 12px", background: T.bg, borderRadius: 9, marginBottom: 6 }}>
            <span style={{ fontSize: 13, flex: 1 }}>General farm visit / checkup</span>
            <Input type="number" placeholder="500" value={fee} onChange={e => setFee(e.target.value)} style={{ width: 90, textAlign: "right" }} />
            <span style={{ fontSize: 12, color: T.gray, minWidth: 60 }}>₹/visit</span>
          </div>
      ...
      {step === 5 && <>
        ...
        <div style={{ ...css.card, padding: 0, overflow: "hidden", marginBottom: 12 }}>
          <div style={{ background: T.blueLight, padding: "18px 20px", display: "flex", gap: 16, alignItems: "center" }}>
            <div style={{ width: 60, height: 60, borderRadius: "50%", background: "#BFDBFE", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Sora', sans-serif", fontWeight: 700, fontSize: 22, color: "#1D4ED8", flexShrink: 0 }}>VET</div>
            <div>
              <div style={{ fontFamily: "'Sora', sans-serif", fontWeight: 700, fontSize: 18 }}>{name || "Dr. Ramesh Yadav"}</div>
              <div style={{ fontSize: 13, color: T.gray, marginBottom: 6 }}>Bovine Specialist · {district}, {state}</div>
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                <Badge label={degree} color={T.blue} bg={T.blueLight} />
                <Badge label={`${experience} yrs exp`} color={T.greenDark} bg={T.greenLight} />
          ...
          <div style={{ padding: "14px 20px" }}>
            {[
              ["🩺","Speciality", animals.map(a=>a.split(" ").slice(1).join(" ")).join(" · ")],
              ["📋","Services", services.slice(0,4).join(" · ")],
              ["🕐","Availability", `${days.join(", ")} · 8:00 AM – 6:00 PM`],
              ["📍","Service area", `${district} district · ${radius}`],
              ["₹","Starting fee", `₹${fee} / visit`],
            ].map(([icon, k, v]) => (
              <div key={k}>
        ...
        <div style={css.row(10)}>
          ...
          <Btn bg="#15803D" color="#fff" full onClick={handleSubmit}>✓ Submit Application</Btn>
        </div>
      </>}
    </div>
  );
...
export default function App() {
  ...
  // Warnings or Info states
  const [isUsingLiveDb, setIsUsingLiveDb] = useState(false);

  const loadData = async () => {
    try {
      const [vetsRes, semenRes, feedRes, medRes, listRes] = await Promise.all([
        fetch('/api/vets').then(r => { if (!r.ok) throw new Error(); return r.json(); }),
        fetch('/api/semen').then(r => { if (!r.ok) throw new Error(); return r.json(); }),
        fetch('/api/feed').then(r => { if (!r.ok) throw new Error(); return r.json(); }),
        fetch('/api/medicine').then(r => { if (!r.ok) throw new Error(); return r.json(); }),
        fetch('/api/listings').then(r => { if (!r.ok) throw new Error(); return r.json(); })
      ]);
      setVets(vetsRes);
      setSemen(semenRes);
      setFeed(feedRes);
      ...
      console.warn("Express backend API is not reachable or not fully seeded. Seamlessly falling back to offline demonstration data.");
  ...
  return (
    <div style={css.app}>
      {/* Dynamic API status flag in top bar */}
      <div style={{ background: isUsingLiveDb ? "#15803D" : "#BA7517", color: "#FFF", fontSize: 11, textAlign: "center", padding: "4px 8px", fontWeight: 700 }}>
        {isUsingLiveDb ? "● CONNECTED TO LIVE NEON POSTGRES DATABASE" : "▲ OFFLINE MODE: CONNECT EXPRESS AND NEON TO ENABLE LIVE PERSISTENCE"}
      </div>
      ...
      <div style={css.main}>
        ...
        {tab === "feed"     && <FeedTab vets={vets} listings={listings} feed={feed} onBook={setBookingVet} onEnquire={setEnquireListing} />}
        {tab === "market"   && <MarketTab listings={listings} onRefresh={loadData} onEnquire={setEnquireListing} />}
        {tab === "semen"    && <SemenTab semen={semen} onAddCart={addToCart} />}
        {tab === "fodder"   && <FodderTab feed={feed} onAddCart={addToCart} />}
        {tab === "medicine" && <MedicineTab medicine={medicine} onAddCart={addToCart} />}
        {tab === "vet"      && <VetTab vets={vets} onBook={setBookingVet} />}
        {tab === "onboard"  && <OnboardTab onRefresh={loadData} />}
      </div>
